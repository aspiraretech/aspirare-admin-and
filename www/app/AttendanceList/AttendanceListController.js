...
angular.module('smartAdminApp')
    .controller('AttendanceListController', function($scope, $state, StudentListFactory, ionicToast, AttendanceListFactory, StudentAttendanceFactory) {

        $scope.student = StudentListFactory.selectedStudent;
        $scope.subject = StudentAttendanceFactory.selected.subject;
        $scope.attendance = [];
        $scope.totalPresent = 0;

        $scope.getAllAttendanceForStudent = function() {
            AttendanceListFactory.getAllAttendance($scope.student.Id, $scope.subject.Id)
